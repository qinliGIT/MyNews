        ...
        try {
            ...
            if (object.getString("reason").equalsIgnoreCase("成功的返回")) {// 解析获取数据
                ...
                for (int i = 0; i < array.length(); i++) {
                    ...
                    news = (NewsAllBean) JsonUtils.putJsonObjectToBean(forBean,
                            "com.ql.mynews.utils.NewsAllBean");
                    list.add(news);
                }
            } else {
                news = null;
    ...
    public static void getRequestForNewsOther(Context context, String type, Handler handler) {
        ...
        try {
            ...
            if (object.getString("reason").equalsIgnoreCase("成功的返回")) {// 解析获取数据
                ...
                for (int i = 0; i < array.length(); i++) {
                    JSONObject forBean = (JSONObject) array.get(i);
                    news = (NewsOtherBean) JsonUtils.putJsonObjectToBean(forBean,
                            "com.ql.mynews.utils.NewsOtherBean");
                    list.add(news);
                }
            } else {
                news = null;
